...
# endif
    public abstract class ParserBase {
        ...
        protected void Set(string filePath, TextReader reader, DiagContext diagCtx, IEnumerable<string> ppSymbols) {
            _lexer = Lexer.Get(filePath, reader, diagCtx, ppSymbols);
            _tokenIndex = -1;
            _filePath = filePath;
            _diagCtx = diagCtx;
        }
        ...
        protected Token GetToken(int index = 0) {
            ...
            while (_tokenIndex < index) {
                ...
                if (!IsTrivalToken(tokenKind)) {
                    if (tokenKind == '#') {
                        var nextToken = GetTokenCore();
                        var nextTokenKind = nextToken.Kind;
                        if (nextTokenKind == (int)TokenKind.Whitespace) {
                            nextToken = GetTokenCore();
                        }
                        if (nextTokenKind != (int)TokenKind.Identifier) {
